...
const env = setupGhRouteTest("stage-cli-pr-routes-");
...
async function writeFakeGh(fixtures: {
	...
	const dir = path.join(env.binDir, "fixtures");
	...
	await env.writeFakeGh(`#!/bin/sh
dir="${dir}"
echo "$@" >> "${env.binDir}/gh-argv.log"
...
function start(): Promise<number> {
	const db = getDb({ dbPath: env.dbPath });
	return env.startWithRoutes(pullRequestRoutes(db));
}
...
describe("pull-request API", () => {
	...
	it("loads a run's targeted PR by number instead of the checked-out branch", async () => {
		...
		const argv = await fs.readFile(path.join(env.binDir, "gh-argv.log"), "utf8");
